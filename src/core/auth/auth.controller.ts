...
export class AuthController {
  ...
  @ApiBody({
    schema: {
      ...
      properties: {
        ...
        address:         { type: 'string', example: '5 شارع التحرير' },
        role:            { type: 'string', enum: ['user', 'delivery', 'admin'] },
      },
    },
  })
  ...
  @ApiResponse({ status: 200, description: 'Email verified, returns JWT token' })
  ...
  @Post('resend-code')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Resend verification code (max once every 5 minutes)' })
  @ApiResponse({ status: 200, description: 'Code resent successfully' })
  @ApiResponse({ status: 400, description: 'Cooldown active — seconds remaining returned' })
  ...
  @ApiResponse({ status: 200, description: 'Reset email sent (same response regardless of email existence)' })
